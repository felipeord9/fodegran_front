...
import { findOneByCredito , mailComite , mailComite2 } from '../../services/estudioCreditoService';
...
export default function EditarPaqueteCredito() {
  ...
  useEffect(() => {
    if(user && (user.role === 'tesoreria' || user.role === 'gerencia' || user.role === 'comite1' || user.role === 'comite2' || user.role === 'presidente' || user.role === 'admin')){
    findOneByCredito(id)
      .then(({data})=>{
        setSuggestions(data)
      })
